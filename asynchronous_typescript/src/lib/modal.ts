...
//This line returns the promise, in turn the caller then waits for it to resolve
...
function closeModal(yesno: 'yes' | 'no') {
    ...
    modalWindow.querySelector('.modal-yes').removeEventListener('click', listenerYes);
    modalWindow.querySelector('.modal-no').removeEventListener('click', listenerNo);
